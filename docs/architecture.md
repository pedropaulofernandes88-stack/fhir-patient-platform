...
5. **Tornar mudanças observáveis:** atualizar `meta.versionId`, `meta.lastUpdated` e
   registrar uma auditoria agregada a cada importação.
...
A chave `(resource_type, resource_id)` garante idempotência: a primeira importação cria
o registro e as seguintes o atualizam, incrementando a versão. O MVP guarda apenas a
versão corrente; um produto real deve manter histórico imutável e políticas de
concorrência.
